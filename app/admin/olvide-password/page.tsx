      ...
      <div className="relative w-full max-w-[400px] bg-white/[0.06] backdrop-blur-2xl border border-white/10 rounded-xl shadow-[0_8px_40px_rgba(0,0,0,0.4)] p-8 flex flex-col gap-6">
        <div className="flex flex-col gap-3 items-center text-center">
          <Image
            ...
            priority
          />
          <span className="text-headline-sm text-white/50">Recuperar contraseña</span>
        </div>

        {state.submitted ? (
          <p className="text-headline-sm text-white/70 bg-white/[0.06] border border-white/10 rounded-md px-4 py-3 text-center">
            Si existe una cuenta con ese usuario o email, te enviamos un link para elegir una nueva contraseña.
            Revisá tu correo.
          </p>
        ) : (
          <form action={formAction} className="flex flex-col gap-4">
            <div className="flex flex-col gap-1.5">
              <label htmlFor="identifier" className="text-title-lg font-medium text-white/60 tracking-wide">
                USUARIO O EMAIL
              </label>
              <input
                id="identifier"
        ...
        <Link
          href="/admin/login"
          className="text-headline-sm text-white/50 hover:text-white text-center transition-colors"
        >
          Volver a ingresar
        </Link>
      </div>
